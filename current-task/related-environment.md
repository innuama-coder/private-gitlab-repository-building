...
| ECS5 | 备份节点 | GitLab 备份接收、校验和保留 | 否，只运行备份基础设施 |

三台机器通过 Tailscale 组成私网。公网用户只访问 ECS4，ECS4 再把请求转发到 Mac Studio。
...
- 执行目标，但只在 M5 启用备份基础设施。
- 接收 Mac Studio 生成的 GitLab 备份。
- 保存备份校验结果和保留版本。

### 已确认环境

- 公网 IPv4：`47.93.189.15`。
- 内网 IPv4：`10.0.0.70`。
- 系统：Ubuntu `26.04`。
- 资源：4 vCPU、7.1GiB 内存、59G 根盘。
- 磁盘可用空间：约 47G（检查时）。
...
这一定义满足冻结六要素中的两个约束：ECS5 首期不参与服务部署，同时备份必须离开 Mac Studio 保存。
...
- ECS4、ECS5 上原有的 `agent-*` 容器、Compose 配置和 Docker 数据不属于首期 GitLab 交付物。
