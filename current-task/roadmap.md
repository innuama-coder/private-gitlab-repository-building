...
交付一套真正可恢复的 GitLab 数据保护能力：先以 LaCie 外置磁盘完成备份、校验和恢复，再补齐与 Mac Studio 故障域分离的正式目标，并能恢复出可使用的项目。

### 02. 工作方法

1. 在 Mac Studio 配置每日 GitLab 备份。
2. 确认 LaCie 候选目标；若要求主机级隔离，再确定远端目标并部署接收目录、专用账号和最小权限。
...
- 正式备份目标必须与 Mac Studio 保持物理或故障域分离；LaCie 当前只满足独立物理介质，不满足主机级故障域分离。
- 备份目录使用专用账号和最小权限。
